...
def train_generator(percent_tag, data_path, modality):
    ...
    for percent in percents:
        if percent_tag == f"_{percent}p":  # 精确匹配，避免 "5" 在 "25p" 中误匹配
            train = torch.load(os.path.join(data_path, f"{modality}train.pt"))
            # print(train.shape)
            length = int((train["samples"].shape[0]*percent)/100)
            print(f"Generating {percent}% training data ... Length: {length}")
            ...
            _, X_sub, _, y_sub = train_test_split(
                 X, y,
                 train_size=length,
                 stratify=y,      # ← 按 y 的类别分层
                 shuffle=True,    # 先打乱数据，避免前几类集中
                 random_state=42  # 固定随机种子，复现
            )
            train_data = {"samples": X_sub, "labels": y_sub}
            # train_data = {
            ...
            labels_np = train_data["labels"].cpu().numpy() if isinstance(train_data["labels"], torch.Tensor) else train_data["labels"]
            unique, counts = np.unique(labels_np, return_counts=True)
            print(f"label distribution ({percent}%):", dict(zip(unique, counts)))
