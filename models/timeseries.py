...
class timeseries(nn.Module):
    def __init__(self, configs, device):
        ...
        self.timestep = configs.timeseries.timesteps
        self.Wk = nn.ModuleList([nn.Linear(configs.timeseries.hidden_dim, self.num_channels) for i in range(self.timestep)])
        self.lsoftmax = nn.LogSoftmax()
        self.device = device

        self.projection_head = nn.Sequential(
            nn.Linear(configs.timeseries.hidden_dim, configs.final_out_channels // 2),
            nn.BatchNorm1d(configs.final_out_channels // 2),
            nn.ReLU(inplace=True),
            nn.Linear(configs.final_out_channels // 2, configs.final_out_channels // 4),
        )

        self.seq_transformer = Seq_Transformer(patch_size=self.num_channels, dim=configs.timeseries.hidden_dim, depth=4,
                                               heads=4, mlp_dim=64)
